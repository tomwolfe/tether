...
Effective values follow strict precedence: mission explicit value > project config > built-in default, applied independently for `recovery.max_attempts`, `verification.commands`, and `verification.timeout_seconds` (and `recovery.strategy`, which additionally auto-escalates to `reset_to_checkpoint` on detected oscillation, dogfood-24). Mission models keep these fields Optional so "unset" is distinguishable from an explicit value.
